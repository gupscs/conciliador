...
public class OrderController {
	...
	// @PreAutorize("hasAnyRole('ORDERS')")
	public ResponseEntity<RestResponseDto<?>> saveOrderCost(@RequestBody OrderCostDto orderCostDto) {
		try {
	...
	// @PreAutorize("hasAnyRole('ORDERS')")
	public ResponseEntity<RestResponseDto<OrderValuesDto>> getOrderValues(@PathVariable(value="id") String orderId) {
		try {
			OrderValuesDto dto = orderService.getOrderValues(orderId);
			return ResponseEntity.ok(new RestResponseDto<OrderValuesDto>(dto));
		} catch (Exception e) {
			log.error("Error to get order values - Id: "+orderId, e);
			return ResponseEntity.badRequest().body(new RestResponseDto<OrderValuesDto>(e.getMessage()));
		}
	}
}
